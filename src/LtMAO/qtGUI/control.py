...
def build_mask_viewer(widget: QWidget):
    ...
    # batch layout
    layout2 = QHBoxLayout()
    layout2.addWidget(QLabel('🔄 Batch value:'))
    batch_line = QLineEdit()
    batch_line.setInputMask('0.0')
    batch_line.setText('0.0')
    batch_line.setMaximumWidth(80)
    layout2.addWidget(batch_line)

    def batch_all():
        if table.rowCount() == 0:
            print('mask_viewer: Error: Load table first before batch.')
            return
        batch_value = batch_line.text()
        for j in range(table.columnCount()):
            for i in range(table.rowCount()):
                table.setItem(i, j, QTableWidgetItem(batch_value))
        print(f'mask_viewer: Finish: Batch all to {batch_value}')

    button = QToolButton()
    button.setText('🔄 Batch All')
    button.clicked.connect(lambda event: batch_all())
    layout2.addWidget(button)
    layout2.addWidget(QLabel('💡 Format: X.Y (1.0, 0.0)'))
    layout2.addStretch()
    layout.addLayout(layout2)
