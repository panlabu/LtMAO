...
def get_shell(shell_id):
    if shell_id == 'Directory':
        return r'Directory\shell'
    elif shell_id == 'No Extension':
        return r'SystemFileAssociations\.\shell'
    elif shell_id == 'wad':
        return r'SystemFileAssociations\.client\shell'
    else:
        return rf'SystemFileAssociations\.{shell_id}\shell'
...
def create_contexts(icon_file):
    # create submenus
    for shell_id in submenus:
        commands = [f'LtMAO.{command_id}' for command_id in submenus[shell_id] if submenus[shell_id][command_id]]
        if len(commands) > 0:
            create_submenu(
            ...
            remove_submenu(get_shell(shell_id))

    # create commands
    with winreg.OpenKeyEx(winreg.HKEY_LOCAL_MACHINE, r'Software\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell') as key:
        for command_id in commands:
            create_command(
                root=key,
                cmd_name=f'LtMAO.{command_id}',
                cmd_desc=commands[command_id]['desc'],
                cmd_value=commands[command_id]['value'],
                icon_file=icon_file
            )
...
def create_shortcut(path, icon_file):
    cmd = [
        ...
        '-Command',
        (
            f'$s=(New-Object -COM WScript.Shell).CreateShortcut("{path}");'
            f'$s.TargetPath="{os.path.abspath(pythonw_file)}";'
            f'$s.WorkingDirectory="{os.path.abspath('.')}";'
            f'$s.Arguments=\'"{os.path.abspath(gui_file)}"\';'
            f'$s.IconLocation="{os.path.abspath(icon_file)}";'
            f'$s.Description="Run LtMAO-pan";'
            f'$s.Save()'
        )
    ]
    p = subprocess.Popen(
...
def create_desktop(icon_file):
    with winreg.OpenKeyEx(winreg.HKEY_CURRENT_USER, r'Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders') as key:
        desktop_file =  rf'{winreg.ExpandEnvironmentStrings(winreg.QueryValueEx(key, "Desktop")[0])}\LtMAO.lnk'
        create_shortcut(desktop_file, icon_file)
        print(f'winLT: Finish: Create Desktop Shortcut: {desktop_file}')

def create_launch(icon_file):
    launch_file = os.path.abspath(r'.\LtMAO.lnk')
    if not os.path.exists(launch_file):
        create_shortcut(launch_file, icon_file)
